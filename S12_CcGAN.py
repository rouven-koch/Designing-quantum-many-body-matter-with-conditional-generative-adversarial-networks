...
#-----------------------------------------------------------------------------
# spin-1/2 model - continuous conditional GAN 
#-----------------------------------------------------------------------------
...
# # load enhanced data 
# n_data = 2250
# x_train = np.load("/u/11/kochr1/unix/Rouven/Python/Project_2/results_paper/S12/x_data_all.npy")
# x_train = scaler.fit_transform(x_train)  
# l_train = np.load("/u/11/kochr1/unix/Rouven/Python/Project_2/results_paper/S12/label_all.npy")
...
data_original = np.load("data/data_S12.npy", allow_pickle=True)
n_data = 2250
# extracting and formating
label_all = np.zeros((2250,4))
dos_all = np.zeros((2250,50,18))
for i in range(2250):
    dummie1 = np.array(data_original[i])
    dummie2 = dummie1[0]
    dos_all[i,:,:] = dummie1[0]    
    label_all[i,:] = dummie1[1]
...
# create generator 
generator_S12 = gen(noise_dim, label_dim, image_dim)
adam = Adam(lr=0.001, beta_1=0.5)
generator_S12.compile(loss='mean_squared_error', optimizer=adam)    

# create discriminator 
discriminator_S12 = disc(image_dim, label_dim)
discriminator_S12.compile(loss='binary_crossentropy', optimizer='SGD')

# create CcGAN
make_trainable(discriminator_S12, False)
gan_S12 = build_gan(generator_S12, discriminator_S12)
gan_S12.compile(loss='binary_crossentropy', optimizer=adam)
...
for epoch in range(num_epochs):
    ...
    for index in range(num_batches):
        # train discriminator
        make_trainable(discriminator_S12, True)
        ...
        discriminator_S12.train_on_batch([image_batch, label_batch], y_real)
        # train discriminator on fake data
        noise_batch      = np.random.normal(0, 1, (batch_size, noise_dim))    
        generated_images = generator_S12.predict([noise_batch, label_batch])
        y_fake           = np.zeros(batch_size) + 0.1 * np.random.uniform(0, 1, size=batch_size)
        d_loss = discriminator_S12.train_on_batch(generated_images, y_fake)  
        make_trainable(discriminator_S12, False)
        # train GAN
        gan_loss = gan_S12.train_on_batch([noise_batch, label_batch, image_batch, label_batch], [y_real, y_fake])
...
# generator_S12.save_weights('generator_CcGAN_MB_18_Sz_new.h5')
# discriminator_S12.save_weights('discriminator_CcGAN_MB_18_Sz_new.h5')
# gan_S12.save_weights('gan_CcGAN_MB_18_Sz_new.h5')

# generator_S12.load_weights('generator_CcGAN_MB_18_S12.h5')
# discriminator_S12.load_weights('discriminator_CcGAN_MB_18_S12.h5')
# gan_S12.load_weights('gan_CcGAN_MB_18_S12.h5')
...
test_label_batch[0,0] = 0.0  # insert here Delta_J scaled in [0.0, 1.0]  
test_label_batch[0,1] = 0.0  # insert here J_2 scaled in [0.0, 1.0] 

# make new prediction
pred = generator_S12.predict([noise_batch, test_label_batch])[0]
pred_scale = scaler.inverse_transform(pred)

# print real conditional parameter values
D_real = scaler_alpha.inverse_transform(test_label_batch)
print('conditional parameter:')
print('Delta_J =',  D_real[0,0])
J_real = scaler_beta.inverse_transform(test_label_batch)
print('J_2 =', J_real[0,1])
...
generator_S12.summary()
discriminator_S12.summary()
gan_S12.summary()