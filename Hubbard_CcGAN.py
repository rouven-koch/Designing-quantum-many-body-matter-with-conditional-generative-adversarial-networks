...
#-----------------------------------------------------------------------------
# Hubbard model - continuous conditional GAN 
#-----------------------------------------------------------------------------
...
# # load enhanced data 
# n_data = 2200
# x_train = np.load("/u/11/kochr1/unix/Rouven/Python/Project_2/results_paper/Hubbard/x_data_all.npy")
# x_train = scaler.fit_transform(x_train)  
# l_train = np.load("/u/11/kochr1/unix/Rouven/Python/Project_2/results_paper/Hubbard/label_all.npy")
...
dos_hubbard = np.load("data/spectra_hubbard.npy", allow_pickle=True)
label_all = np.load("data/cparams_hubbard.npy", allow_pickle=True)
n_data = 2200

# format to 3D
d3_data = np.zeros((n_data, 50, 18))
for k in range(n_data):
    for j in range(18):
        for i in range(50):
            d3_data[k,i,j]=dos_hubbard[k,  (j*50+i)]


dos_mirrow = np.flip(d3_data, axis=2)
label_all = np.concatenate((label_all, label_all))
dos_all_new = np.concatenate((d3_data, dos_mirrow))
# flatten zs
...
# create generator 
generator_H = gen(noise_dim, label_dim, image_dim)
adam = Adam(lr=0.001, beta_1=0.5)
generator_H.compile(loss='mean_squared_error', optimizer=adam)    

# create discriminator 
discriminator_H = disc(image_dim, label_dim)
discriminator_H.compile(loss='binary_crossentropy', optimizer='SGD')

# create CcGAN
make_trainable(discriminator_H, False)
gan_H = build_gan(generator_H, discriminator_H)
gan_H.compile(loss='binary_crossentropy', optimizer=adam)
...
# ! extra information: train with batch_size=50 and num_epochs=10 afterwards again !

for epoch in range(num_epochs):
    print("Epoch {}/{}".format(epoch + 1, num_epochs))
    for index in range(num_batches):
        # train discriminator
        make_trainable(discriminator_H, True)
        ...
        discriminator_H.train_on_batch([image_batch, label_batch], y_real)
        # train discriminator on fake data
        noise_batch      = np.random.normal(0, 1, (batch_size, noise_dim))    
        generated_images = generator_H.predict([noise_batch, label_batch])
        y_fake           = np.zeros(batch_size) + 0.1 * np.random.uniform(0, 1, size=batch_size)
        d_loss = discriminator_H.train_on_batch(generated_images, y_fake)  
        make_trainable(discriminator_H, False)
        # train GAN
        gan_loss = gan_H.train_on_batch([noise_batch, label_batch, image_batch, label_batch], [y_real, y_fake])
...
# generator_H.save_weights('generator_CcGAN_MB_hubbard.h5')
# discriminator_H.save_weights('discriminator_CcGAN_MB_hubbard.h5')
# gan_H.save_weights('gan_CcGAN_hubbard.h5')

# generator_H.load_weights('generator_CcGAN_MB_hubbard.h5')
# discriminator_H.load_weights('discriminator_CcGAN_MB_hubbard.h5')
# gan_H.load_weights('gan_CcGAN_hubbard.h5')
...
test_label_batch[0,0] = 0.5  # insert here U  scaled in [0.0, 1.0]  
test_label_batch[0,1] = 0.5  # insert here mu scaled in [0.0, 1.0] 

# make new prediction
pred = generator_H.predict([noise_batch, test_label_batch])[0]
pred_scale = scaler.inverse_transform(pred)

# print real conditional parameter values
U_real = scaler_alpha.inverse_transform(test_label_batch)
print('conditional parameter:')
print('U =',  U_real[0,0])
mu_real = scaler_beta.inverse_transform(test_label_batch)
print('mu =', mu_real[0,1])
...
plt.contourf(xs,ys_hubbard,d3_plot_gan,100)
